...
## Known issue: `npm ci` / `npm test` cannot currently run as written

Two defects in `package.json` sit outside the files this guard's
documentation unit is scoped to change (`README.md` and `index.html` only),
so they are recorded here rather than silently worked around:

- **No `package-lock.json` is committed.** `npm ci` requires an existing lock
  file and fails immediately without one (`npm error The 'npm ci' command can
  only install with an existing package-lock.json...`). The fix is to commit
  a lock file generated by `npm install` against the current `package.json`.
- **The `test` script's `node --test test/` does not recurse into the
  directory.** On the Node version this project targets, an explicit path
  argument to `node --test` is resolved as a literal module specifier rather
  than searched, so `node --test test/` fails immediately with
  `Cannot find module '.../test'` — before any test file is ever loaded, and
  independent of whether dependencies are installed. The fix is a glob that
  matches only the test-bearing files, e.g.
  `node --test 'test/**/*.test.js'` (a bare `node --test` with no path also
  works, but additionally picks up every `.mjs` file under `test/`, including
  the non-test helpers, as a pseudo test file).

With both applied — `npm install` run once to produce a lock file, and the
`test` script's glob corrected — the suite passes 24/24 against this
`index.html`, confirmed by hand against the source and the built copy.
